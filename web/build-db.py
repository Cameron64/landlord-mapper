...
def build_filings(cx, st, by_parcel):
    ...
    # regardless, so an unfiltered histogram counted 7,345 answered out-of-scope
    # owners into matched/no_record/not_resolved while the denominator stayed
    # in-scope only. The three published shares then summed to 110.5% and the
    # headline "no Texas filing" share read 61.9% instead of 51.8%.
    ...
    # answered or not. The pages that quote it mean exactly that, and the old
    # bucket under-reported it by the same 7,345.
